...
namespace GameAIStudent
{
    ...
    public class ShotSelection
    {
            ...
            )
        {
            ...
            var delta = interceptPos - thisMinion.HeldBallPosition;
            //Debug.Log(delta.magnitude);
            if ((opponent.Vel.magnitude != thisMinion.MaxPathSpeed && opponent.PrevVel != opponent.Vel && delta.magnitude > 15.0f) || delta.magnitude > 30f
              )
            {
            ...
            var hitWall = NavMesh.Raycast(opponent.Pos, interceptPos, out NavMeshHit hit, opponentNavmask);
            //Debug.DrawLine(opponent.Pos, interceptPos, Color.green);
            if (hitWall && opponentVel.magnitude != 0)
            {
            ...
            //var projectedAndPerpendicularSide1 = (new Vector3(-projectileDir.z, 0f, projectileDir.x)).normalized;
            // var projectedAndPerpendicularSide2 = -projectedAndPerpendicularSide1;
           // var left = thisMinion.HeldBallPosition + (0.25f * projectedAndPerpendicularSide1);
            //var right = thisMinion.HeldBallPosition + (0.25f * projectedAndPerpendicularSide2);
            
        
           if (Physics.SphereCast(thisMinion.HeldBallPosition, 0.25f, delta.normalized, out RaycastHit hit1, delta.magnitude, mask))
        ...
        // direction of spherecast should be between distance of target to the initial position
        // magnitude may be the same as well try interceptPos and minPos
